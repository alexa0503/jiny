...
class GenerateMenus
{
    ...
    public function handle($request, Closure $next)
    {
        \Menu::make('menu', function ($menu) {
            ...
            $item = $menu->add('新闻资讯', ['url'=>route('post.index') ,'class'=>'openable bg-palette3']);
            $item->add('查看', ['url'=>route('post.index'),'class'=>'bg-palette3']);
            $item->add('添加', ['url'=>route('post.create'),'class'=>'bg-palette3']);
